 ...
 *   int            cTarget3_send( const int8_t* buf, int8_t len );
 *   int            cTarget3_sendMessage( const char_t* buf, int8_t len );
 *   int            cTarget3_sendStruct( const struct target_struct* data );
 *   int            cTarget3_receiveMessage( char_t* buf, int8_t len );
 *   [dynamic, optional]
  ...
  if( !strcmp(signature_path, "sTarget1" ) ){
    ...
    cTarget3_set_descriptor( Target3Desc );
      if( !strcmp( function_path, "send" ) ){
        ...
        if( exp_val->data.mem_int == cTarget3_send( arguments[0].data.mem_int8_t_buf, arguments[1].data.mem_int8_t ) ){
          printf("\nResult：OK\n");
        }else{
          printf("\nResult：NG\n");
        }
        ...
        if( exp_val->data.mem_int == cTarget3_sendMessage( arguments[0].data.mem_char_t_buf, arguments[1].data.mem_int8_t ) ){
          printf("\nResult：OK\n");
        }else{
          printf("\nResult：NG\n");
        }
        ...
        // if( exp_val->data.mem_int == cTarget3_sendStruct( arguments[0].data.mem_struct target_struct_buf ) ){
        //   printf("\nResult：OK\n");
        // }else{
        //   printf("\nResult：NG\n");
        // }
        ...
        if( exp_val->data.mem_int == cTarget3_receiveMessage( VAR_out_char[0], arguments[1].data.mem_int8_t ) ){
          printf("\nResult：OK\n");
        }else{
          printf("\nResult：NG\n");
        }
